...
(function () {
  ...
  function resetForm(formElement) {
    const formInputCollection = formElement.querySelectorAll(
...
(function () {
  ...
  function privacyInputHandler(evt) {
    ...
    const button = target
      .closest('.checkbox')
      .previousElementSibling.querySelector('.btn');
    if (target.checked) {
      ...
      if (this.id === 131) {
        return {
          ...
          customer: {
            ...
            telephone: (this.getFormValue(3) + this.getFormValue(14)).replace(
              /[^0-9.]/g,
              ''
            ),
          },
      ...
      if (this.id === 859) {
        return {
          ...
          customer: {
            ...
            telephone: (this.getFormValue(4) + this.getFormValue(15)).replace(
              /[^0-9.]/g,
              ''
            ),
          },
      ...
      if (this.id === 1839) {
        return {
          ...
          customer: {
            ...
            telephone: (this.getFormValue(3) + this.getFormValue(9)).replace(
              /[^0-9.]/g,
              ''
            ),
          },
...
(function () {
  ...
  class Payment {
    ...
    updatePrices(index, isSale = false) {
      ...
      if (isSale) {
        ...
        this.setTotalPrice(
          (
            this.getSalePrice() / 3 -
            (this.getSalePrice() / 3) * SCHOOL_SALE_VALUE
          ).toFixed(2)
        );
    ...
    changeInputPrice(index, isSale = false, promocode = {}) {
        ...
        case 0:
          if (isSale) {
            ...
            if (selectCourseElement.dataset.installment === `true`) {
              ...
              this.setTotalPrice(
                this.getSalePrice() - this.getSalePrice() * SCHOOL_SALE_VALUE
              );
            }
            this.setSaleType(`Я студент-очник / я раньше уже учился у вас`);
            this.setSaleValue(10);
            ...
            if (window.paymentSelect.instance.getPaymentType() === 'payment') {
              ...
              window.paymentSelect.instance.getPaymentType() === 'certificate'
            ) {
            ...
            if (selectCourseElement.dataset.installment === `true`) {
              ...
              window.paymentSelect.instance.getPaymentType() === 'payment'
            ) {
      ...
      if (window.paymentSelect.instance.getPaymentType() === 'payment') {
        let totalInputElement = jQuery('.payment-section')
          .eq(index)
          .find('input[name="wsb_total"]');
        ...
        totalInputElement.nextElementSibling.classList.add(
          'form__label_active'
        );
    ...
    changeCurenciesPrice(index) {
      let totalPriceInUsd = (
        this.getTotalPrice() / parseFloat(rates['usd'])
      ).toFixed(0);
      let totalPriceInRub = (
        this.getTotalPrice() /
        (parseFloat(rates['rub']) / 100)
      ).toFixed(0);
    ...
    updateEripPrice(
      ...
      const eripPaymentSectionCollection =
        document.querySelectorAll(`.payment-section`);
      const eripPaymentPriceElement = eripPaymentSectionCollection[
        paymentIndex
      ].querySelector(`.erip-payment__price-value`);
      ...
      if (eripPaymentPriceElement) {
        ...
        if (Object.values(isPromocode).length && isInstallment) {
          eripPaymentPriceValue = `${(
            (this.getSalePrice() - +window.promocodeData.value) /
            3
          ).toFixed(
            2
          ...
          this.setTotalPrice(
            ((this.getSalePrice() - +window.promocodeData.value) / 3).toFixed(0)
          );
          ...
          eripPaymentPriceValue = `<span class="erip-payment__price-value-old">${this.getSalePrice()}</span> ${
            this.getSalePrice() - this.getSalePrice() * SCHOOL_SALE_VALUE
          } BYN`;
          this.setTotalPrice(
            this.getSalePrice() - this.getSalePrice() * SCHOOL_SALE_VALUE
    ...
    updateInstallmentPrice() {
      ...
      const installmentPaymentPriceElement = document.querySelector(
        `.installment-payment__price-value`
      );
      const installmentPaymentIdInput = document.querySelector(
        `input[name="installment-id"]`
      );
      const installmentPaymentPriceInput = document.querySelector(
        `input[name="installment-price"]`
      );
      const installmentPaymentCourseName = document.querySelector(
        `input[name="installment-course-name"]`
      );
      installmentPaymentIdInput.value =
        utilsModule.getRandId() + INSTALLMENT_SHOP_ID;
      installmentPaymentPriceInput.value = this.getSalePrice();
      installmentPaymentCourseName.value = courseData.installmentProductName;
      const installmentDebtPayment = this.getInstallmentDebt(
        this.getSalePrice(),
        INSTALLMENT_TERM
      );
      installmentPaymentPriceElement.textContent = `${(
        (this.getSalePrice() + installmentDebtPayment) /
        INSTALLMENT_TERM
      ).toFixed(2)} BYN x ${INSTALLMENT_TERM} месяцев ≈ ${(
        this.getSalePrice() + installmentDebtPayment
      ).toFixed(2)} BYN`;
    }
    recalculateInstallmentPrice(installmentTerm) {
      ...
      installmentPaymentPriceElement.textContent = `${(
        (this.getSalePrice() + installmentDebtPayment) /
        installmentTerm
      ).toFixed(2)} BYN x ${installmentTerm} месяцев ≈ ${(
        this.getSalePrice() + installmentDebtPayment
      ).toFixed(2)} BYN`;
    }
    getInstallmentDebt(coursePrice, installmentTerm = 12) {
      ...
      let installmentDebtPayment =
        coursePrice * (installmentMonthPercentageDebt / 100);
      while (installmentTerm) {
        installmentDebtPayment +=
          (coursePrice - installmentMonthPayment) *
          (installmentMonthPercentageDebt / 100);
        installmentTerm--;
  ...
  class PaymentMethod {
    ...
    constructor(paymentMethodsContainer) {
      ...
      if (this._el) {
        ...
        this._el.addEventListener(
          `click`,
          this.onPaymentMethodsClickHandler.bind(this)
        );
      }
    }
    ...
    onPaymentMethodsClickHandler(evt) {
      ...
      if (target.matches(`input`)) {
        ...
        utilsModule.removeClass(
          paymentSections,
          'payment-section_state-active'
        );
        paymentSections[this._paymentMethodIndex].classList.add(
          'payment-section_state-active'
        );
        // ERIP
    ...
    renderPaymentMethods(container, data) {
      ...
      for (const paymentMethodData of data) {
        const paymentMethodElement =
          this.createPaymentMethodElement(paymentMethodData);
    ...
    createPaymentMethodElement(data) {
      ...
      const duplicatedPaymentMethodElement =
        paymentMethodTemplate.cloneNode(true);

      if (!data.hidden) {
        duplicatedPaymentMethodElement.querySelector(`input`).value = data.id;
        duplicatedPaymentMethodElement
          .querySelector(`input`)
          .setAttribute(`data-payment`, data.name);
        duplicatedPaymentMethodElement
          .querySelector(`.payment-item__name`)
          .insertAdjacentHTML('afterbegin', data.title);
        data.checked
          ? (duplicatedPaymentMethodElement.querySelector(
              `input`
            ).checked = true)
          : null;
    ...
    changePaymentMethod(paymentMethodIndex) {
      if (this._el) {
        this._el.querySelector(
          `input[name="payment"][value="${paymentMethodIndex}"]`
        ).checked = true;
      }
    }
  }
  ...
  class PaymentSelect {
    ...
    constructor(paymentSelectContainer) {
      ...
      if (this._el) {
        ...
        this._el.addEventListener(
          `click`,
          this.onPaymentSelectClickHandler.bind(this)
        );
      }
    }
    ...
    onPaymentSelectClickHandler(evt) {
      const target = evt.target;
      const courseListElements = this._el.querySelectorAll(
        `.ums-select__list-item`
      );

      if (target.matches(`button`)) {
        this._el
          .querySelector(`.ums-select__btn`)
          .classList.toggle(`ums-select__btn_state-active`);
        this._el
          .closest(`.form__select`)
          .classList.toggle(`form__select_state-active`);
        this._el
          .querySelector(`.ums-select__list`)
          .classList.toggle(`ums-select__list_visibility-open`);
      }
      if (target.matches(`li`)) {
        const paymentSections = document.querySelectorAll(`.payment-section`);
        // RESET PAYMENT'S OPTIONS
        const paymentMethodInputCollection =
          document.querySelectorAll(`.payment-item__input`);
        ...
        utilsModule.removeClass(
          courseListElements,
          `ums-select__list-item_state-active`
        );
        ...
        this._el
          .querySelector(`.ums-select__list`)
          .classList.remove(`ums-select__list_visibility-open`);
        ...
        paymentModule.changeInputPrice(
          paymentMethodModule.getPaymentMethodIndex()
        );
      }
    }
    ...
    fillCourseData(courseListElement) {
      this._courseData.installmentProductName =
        courseListElement.dataset.courseName;
  ...
  class Promocode {
    ...
    constructor(promocodeContainer) {
      ...
      for (const promocodeElement of Array.from(this._el)) {
        promocodeElement.addEventListener(
          `click`,
          this.onPromocodeElementClickHandler.bind(this)
        );
      }
      // if (this._el) {
    ...
    onPromocodeElementClickHandler(evt) {
      ...
      const paymentType = target
        .closest(`.promocode`)
        .querySelector(`input[name="promocode"]`).dataset.payment;

      if (target.matches('input[name="promocode-toggle"]')) {
        const promocodeInputElement = target.parentElement.nextElementSibling;
        if (paymentType && paymentType === `erip`) {
          if (
            promocodeInputElement.classList.contains(
              `promocode-input_state-success`
            )
          ) {
            const eripPaymentSaleField = sectionElement.querySelector(
              `input[name="sale-school"]`
            ).parentElement;
            const eripPaymentInstallmentField = sectionElement.querySelector(
              `input[name="installment-school"]`
            );
            sectionElement
              .querySelector(`.erip-payment__message-note`)
              .classList.remove(`erip-payment__message-note_active`);
            eripPaymentSaleField.classList.remove(`checkbox_disabled`);
            if (eripPaymentInstallmentField.checked) {
              paymentInstance.updateEripPrice(
                {},
                false,
                true,
                paymentMethod.getPaymentMethodIndex()
              );
            } else {
              paymentInstance.updateEripPrice(
                {},
        ...
        target
          .closest(`.promocode`)
          .querySelector(`input[name="promocode"]`).value = '';
        promocodeInputElement.classList.remove(this._promocodeClasses.success);
        promocodeInputElement.classList.remove(this._promocodeClasses.error);
        promocodeInputElement.classList.toggle(this._promocodeClasses.active);
        ...
        if (paymentSelect.getPaymentType() === 'payment') {
          if (
            promocodeInputElement.closest('.payment-form__section-grid') &&
            promocodeInputElement
              .closest('.payment-form__section-grid')
              .querySelector('.webpay-form__sale-checkbox')
          ) {
            promocodeInputElement
              .closest('.payment-form__section-grid')
              .querySelector('.webpay-form__sale-checkbox')
            ...
            promocodeInputElement
              .closest('.payment-form__section-grid')
              .querySelector('.webpay-form__sale-checkbox')
              .classList.toggle('webpay-form__sale-checkbox_state-disabled');
          }
          paymentInstance.changeInputPrice(
            paymentMethod.getPaymentMethodIndex(),
            false,
            false
          );
      ...
      if (target.matches('button')) {
        ...
        const promocodesData = this.getPromocodesData(
          this._promocodeRequestSettings
        );
        promocodesData
          .then((resp) => resp.json())
          .then((data) => {
            data = Object.values(data);
            target.textContent = defaultButtonText;
            promocodeInputElement.classList.remove(
              this._promocodeClasses.progress
            );
            if (data.length) {
              const modifiedData = inputValue
                .replace(/\s+/gi, '')
                .toUpperCase();
              const result = data.find(
                (item) => item.name.toUpperCase() === modifiedData
              );
              if (result) {
                // MAKE PROMOCODE GLOBAL
                window.promocodeData = result;
                if (paymentType && paymentType === `erip`) {
                  ...
                  const eripPaymentInstallmentField =
                    sectionElement.querySelector(
                      `input[name="installment-school"]`
                    );
                  sectionElement
                    .querySelector(`.erip-payment__message-note`)
                    .classList.add(`erip-payment__message-note_active`);
                  eripPaymentSaleField.classList.add(`checkbox_disabled`);
                  if (eripPaymentInstallmentField.checked) {
                    paymentInstance.updateEripPrice(
                      {
    ...
    showMessage(element, state) {
      if (state == 'success') {
        ...
        element.querySelector('.form__error-label').textContent =
    ...
    _itiOptionsData = {
      ...
      geoIpLookup: (success, failure) => {
        ...
        const localStorageData = JSON.parse(
          localStorage.getItem('ums-country-code')
        );
        if (localStorageData) {
      ...
      customPlaceholder: (selectedCountryPlaceholder) => {
        const customPlaceholder = selectedCountryPlaceholder.replace(
          /[0-9]/g,
          '_'
        );
        ...
        .then((data) => {
          if (data.status === `OK`) {
            const installmentPaymentForm =
              document.querySelector(`.installment-form`);
            const paymentSectionCollection =
              document.querySelectorAll(`.payment-section`);
            ...
            globalUtils.removeClass(
              paymentSectionCollection,
              `payment-section_state-active`
            );
            ...
            alert(
              'Извините, ошибка регистрации заявки. Сообщите менеджеру школы о возникшем вопросе.'
            );
          }
        })
  ...
  if (typeof swiperInstance === `object`) {
    ...
    swiperInstances.forEach((swiperInstanceItem, index) => {
      ...
      new Swiper(`.inner-carousel-instance-${index}`, {
        ...
        navigation: {
          prevEl: swiperInstanceItem.parentElement.querySelector(
            `.inner-carousel__btn-prev`
          ),
          nextEl: swiperInstanceItem.parentElement.querySelector(
            `.inner-carousel__btn-next`
          ),
        },
      });
    });
...
function onCoursesTabsButtonsClickHandler(evt) {
  ...
  jQuery
    ...
    .then(
      (resp) => {
        setTimeout(function () {
          coursesListGrid.classList.remove(
            `course-list__grid_state-is-loading`
          );
...
function onPageTemplateContainerClickHandler(evt) {
  ...
  if (target.matches(`.content-list__title`)) {
    ...
    target.nextElementSibling.classList.toggle(
      'content-list__text_state-actived'
    );
  }
}
function onContactPageInfoContainerClickHandler(evt) {
  ...
  if (target.matches(`.contact-page__info-item`)) {
    const contactPageAdresses = document.querySelectorAll(
    ...
    globalUtils.removeClass(
      contactPageAdresses,
      `contact-page__info-item_active`
    );
...
function onSortNavigationContainerClickHandler(evt) {
  ...
  if (target.matches('.sort-navigation__button')) {
    ...
    globalUtils.removeClass(
      ...
      'sort-navigation__button_state-active'
    );
...
const mobileOptionsMenuOpenButton =
  document.querySelector(`.m-options__menu-btn`);
...
const sortNavigationButtons = document.querySelectorAll(
  `.sort-navigation__button`
);
const sortListContainer = document.querySelector(`.sort-list`);
const innerCarouselElements = document.querySelectorAll(
  `.inner-carousel__grid`
);
const selectCourseBox = document.querySelector(`.ums-select`);
const manualInputContainer = document.querySelector(
  `input[name="manual-price"]`
);
...
document.addEventListener(`DOMContentLoaded`, () => {
  ...
  if (manualInputContainer) {
    manualInputContainer.addEventListener(
      `input`,
      onManualInputContainerInputHandler
    );
  }
  if (manualInputContainer) {
    manualInputContainer.addEventListener(
      `keyup`,
      onManualInputContainerKeyupHandler
    );
  }
  ...
  const elPricePerMonth = document.querySelector(
    '.price-box__item-value .ums-currency__value'
  );
  if (priceBox && !priceBox.classList.contains('online-course__value')) {
	  if (elPricePerMonth) {
		let coursePrice = document.querySelector(
		  '.price-box__item-value'
		).innerText;
  ...
  if (pageTemplateContainer) {
    pageTemplateContainer.addEventListener(
      `click`,
      onPageTemplateContainerClickHandler
    );
  }
  if (contactPageInfoContainer) {
    contactPageInfoContainer.addEventListener(
      `click`,
      onContactPageInfoContainerClickHandler
    );
  }
  if (sortNavigationContainer) {
    sortNavigationContainer.addEventListener(
      `click`,
      onSortNavigationContainerClickHandler
    );
  }
  if (testBtn) {
...
document.addEventListener('click', (evt) => {
  ...
  if (
    ...
    selectCourseBox
      .querySelector(`.ums-select__btn`)
      .classList.contains(`ums-select__btn_state-active`) &&
    !target.closest(`.ums-select`)
  ) {
    selectCourseBox
      .querySelector(`.ums-select__btn`)
      .classList.remove(`ums-select__btn_state-active`);
    selectCourseBox
      .querySelector(`.ums-select__list`)
      .classList.remove(`ums-select__list_visibility-open`);
    selectCourseBox
      .closest(`.form__select`)
      .classList.remove(`form__select_state-active`);
  }
  // ERIP
  if (target.matches(`input[name="installment-school"]`)) {
    const eripPaymentSaleField = document.querySelector(
      `input[name="sale-school"]`
    );
    const eripPaymentPromocodeField =
      document.querySelector(`.promocode-input`);
    if (target.checked) {
      if (eripPaymentSaleField.checked) {
        ...
        eripPaymentPromocodeField.classList.contains(
          `promocode-input_state-success`
        )
      ) {
  ...
  if (target.matches(`input[name="sale-school"]`)) {
    const eripPaymentInstallmentField = document.querySelector(
      `input[name="installment-school"]`
    );
    document
      .querySelector(`.b-promocode`)
      .classList.toggle(`b-promocode_disabled`);
    document
      .querySelector(`.erip-payment__message-note`)
      .classList.toggle(`erip-payment__message-note_active`);
    if (target.checked) {
      if (eripPaymentInstallmentField.checked) {
        paymentInstance.updateEripPrice(
          {},
  ...
  if (target.matches(`input[name="installment-length"]`)) {
    const installmentPaymentTermInput = document.querySelector(
      `input[name="installment-term"]`
    );
...
(function () {
  ...
  const transferMessage = 'Перенаправляем на оплату... test';
  if (button) {
    console.log(button);
    const defaultText = button.textContent;
    requiredInputsCollection = button
      .closest('.form')
    ...
    button.addEventListener('click', function () {
      ...
      if (deliveryValue === 0) {
        ...
        deliveryElement.parentElement
          .querySelector('.form__error-label')
          .classList.remove('form__error-label_active');
        let deliveryAddressElement =
          deliveryElement.parentElement.nextElementSibling.querySelector(
            'input'
          );
        ...
        if (deliveryAddress === '') {
          addErrorClass(
            ...
            deliveryAddressElement.parentElement
          );
          // validation = false;
          selectValidation = false;
        } else {
      ...
      if (
        ...
        if (
          document
            .querySelector('.promocode-input')
            .classList.contains('promocode-input_state-success')
        ) {
        ...
        jQuery(certificateForm)
          .find('input[name="ums-course"]')
          .val(jQuery('.ums-select__btn').text());
        jQuery(certificateForm)
          .find('input[name="ums-price"]')
          .val(jQuery('input[name="total"]').val());
        jQuery(certificateForm)
          .find('input[name="ums-name"]')
          .val(jQuery('input[name="name"]').val());
        jQuery(certificateForm)
          .find('input[name="ums-email"]')
          .val(jQuery('input[name="email"]').val());
        jQuery(certificateForm)
          .find('input[name="ums-tel"]')
          .val(jQuery('input[name="tel"]').val());
        jQuery(certificateForm)
          .find('input[name="ums-delivery"]')
          .val(
            jQuery('select[name="delivery"] option:selected').text() +
              ', ' +
              jQuery('input[name="delivery-address"]').val()
          );
